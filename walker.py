...
class walker():
    def __init__(self, space, pos, ul, ll, w, lua, lla, rua, rla):
        ...
        self.l_knee = pymunk.PivotJoint(self.lul.body, self.lll.body, ll_pos)

        # Create right leg
        self.rul = self._create_leg(pos, ul, w, rua)
        rl_pos = Vec2d(0, -ul).rotated(rua) + pos
        self.rll = self._create_leg(rl_pos, ll, w, rua + rla)
        self.r_knee = pymunk.PivotJoint(self.rul.body, self.rll.body, rl_pos)
        ...
        self.r_limit = pymunk.RotaryLimitJoint(
            self.rul.body, self.rll.body, (-pi / 360.0) - rla, pi - rla)
    ...
    def __del__(self):
        ...
        for constraint in [self.l_knee, self.r_knee, self.hip, self.l_limit, self.r_limit]:
